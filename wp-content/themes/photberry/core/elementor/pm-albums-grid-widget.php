...
class PM_Albums_Grid_Widget extends Widget_Base {

    public function get_name() {
        return 'pm-albums-grid';
    }

    public function get_title() {
        return esc_html__('Albums Grid', 'photberry');
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            'section_albums_grid',
            [
                'label' => esc_html__('Albums Grid', 'photberry')
            ]
        );
    ...
    protected function render() {
        ...
        <div class="photberry_front_end_display">
			...
			if ($filter_status == 'show') {
				echo '<div class="photberry_grid_filter_wrapper">' . photberry_albums_grid_filtering($filter_categ_array) . '</div>';
			}
			$masonry_class = '';
			if ($masonry == 'on') {
			...
			wp_enqueue_script('imagesloaded', array('isotope'), false, true);
			?>
			<div class="photberry_albums_listing_cont photberry_albums_grid photberry_albums_grid<?php echo esc_attr($columns);?> view_type_grid photberry_isotope_trigger <?php echo esc_attr($masonry_class); ?>" data-setPad="<?php echo esc_attr($items_padding); ?>"></div>
			<?php

			$albums_args = '{"output_template": "albums_grid", "post_type": "pm-albums", "post_status":"publish", "posts_first_load": ' . esc_attr($posts_per_page) . ', "posts_per_page": ' . esc_attr($posts_per_click) . ', "offset": 0, "orderby": "' . esc_attr($order_by) . '", "order": "' . esc_attr($order) . '", "posts_counter": 1, "columns": ' . esc_attr($columns) . ', "images_shape": "'. esc_attr($images_shape) .'",  "masonry": "'. esc_attr($masonry) .'", "ajax_callback_function": "photberry_albums_grid_setup", "cat_string": "'. $cat_string .'"}';
		
			?>
			<div class="clear"></div>

			<div class="photberry_load_more_button_wrapper element_albums_grid">
				<a class='photberry_button photberry_load_more_button photberry_load_more photberry_ajax_isotope photberry_ajax_query_posts <?php echo (($load_more_status == 'hide') ? 'photberry_hidden_cont' : '') ?>'
				   ...
				   data-return-to='photberry_albums_listing_cont'
