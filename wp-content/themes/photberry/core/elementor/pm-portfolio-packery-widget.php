...
class PM_Portfolio_Packery_Widget extends Widget_Base {

    public function get_name() {
        return 'pm-portfolio-packery';
    }

    public function get_title() {
        return esc_html__('Portfolio Packery', 'photberry');
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            'section_portfolio_packery',
            [
                'label' => esc_html__('Portfolio Packery', 'photberry')
            ]
        );
    ...
    protected function render() {
        ...
        <div class="photberry_front_end_display">
			...
			if ($filter_status == 'show') {
				echo '<div class="photberry_packery_filter_wrapper">' . photberry_portfolio_grid_filtering($filter_categ_array) . '</div>';
			}
			...
			?>
			<div class="photberry_portfolio_packery_wrapper">
				<div class="photberry_portfolio_packery_cont photberry_portfolio_packery photberry_isotope_trigger is_packery" data-setPad="<?php echo esc_attr($items_padding); ?>"></div>
			</div>
			<?php

			$portfolio_args = '{"output_template": "portfolio_packery", "post_type": "pm-portfolio", "post_status":"publish", "posts_first_load": ' . esc_attr($posts_per_page) . ', "posts_per_page": ' . esc_attr($posts_per_click) . ', "offset": 0, "orderby": "' . esc_attr($order_by) . '", "order": "' . esc_attr($order) . '", "posts_counter": 1, "ajax_callback_function": "photberry_portfolio_packery_setup", "cat_string": "'. $cat_string .'"}';

			?>
			<div class="clear"></div>

			<div class="photberry_load_more_button_wrapper element_portfolio_packery">
				<a class='photberry_button photberry_load_more_button photberry_load_more photberry_ajax_isotope photberry_ajax_query_posts <?php echo (($load_more_status == 'hide') ? 'photberry_hidden_cont' : '') ?>'
				   ...
				   data-return-to='photberry_portfolio_packery_cont'
