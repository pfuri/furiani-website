...
$photberry_fonts_array = json_decode('
    {
...
function photberry_get_all_fonts_name()
{
    global $photberry_fonts_array;

    if (is_array($photberry_fonts_array['items'])) {
        foreach ($photberry_fonts_array['items'] as $key => $font) {
...
# Register Fonts
add_action('wp_enqueue_scripts', 'photberry_register_theme_fonts');
function photberry_register_theme_fonts()
{
    $fonts_array = array();

    # Menu Font
    $fonts_array[photberry_get_theme_mod('header_menu_font_family')] = ':400,' . photberry_get_theme_mod('header_menu_font_weight');

    # Sub Menu Font
    $fonts_array[photberry_get_theme_mod('header_sub_menu_font_family')] = ':300';

    # Main Font
    $fonts_array[photberry_get_theme_mod('main_font_family')] = ':300,400,700,' . photberry_get_theme_mod('main_font_weight') .','. photberry_get_theme_mod('dropcap_font_weight');

    # Headings Font
    $fonts_array[photberry_get_theme_mod('headings_font_family')] = ':400,700,900';

    # Font For Text Logo
    if (photberry_get_theme_mod('logo_type') == 'text_logo') {
        $fonts_array[photberry_get_theme_mod('logo_text_font')] = ':400,700,' . photberry_get_theme_mod('logo_text_weight');
    }

    $fonts = '';
    foreach ($fonts_array as $font_name => $font_params) {
        $fonts .= $font_name . $font_params . "|";
    }
    $font_url = add_query_arg('family', urlencode(substr($fonts, 0, -1)), "//fonts.googleapis.com/css");

    wp_enqueue_style('photberry-fonts', $font_url, array());
}