...
if (!post_password_required()) {
	...
	if (is_array($terms)) {
		foreach ($terms as $term) {
			...
			array_push($categories_w_link, '<a href="' . esc_url(get_term_link($term->slug, $port_tax)) . '">' . esc_html($term->name) . '</a>');
		}
	}
			...
			<div class="photberry_single_post photberry_content_wrapper row <?php echo esc_attr($photberry_sidebar_position); ?>">
				<div class="photberry_content <?php echo esc_attr($content_col_class); ?>">
					<div class="photberry_post_meta">
						<?php /*<div class="photberry_post_meta_item">
							<?php echo esc_html(get_the_date()); ?>
						</div>*/ ?>
						<?php if (photberry_get_post_option('post_client_field_status', 'no') == 'yes' && photberry_get_post_option('post_client_field') !== '') { ?>
						<div class="photberry_post_meta_item">
							<?php echo esc_html__('Client', 'photberry') . ': '. esc_attr(photberry_get_post_option('post_client_field')); ?>
						</div>						
						<?php } ?>
						<div class="photberry_post_meta_item">
							<?php /*echo esc_html__('in', 'photberry'); echo ' ';*/ echo (is_array($categories_w_link) ? join(', ', $categories_w_link) : esc_html('Uncategorized', 'photberry')); ?>
						</div>
						<?php if (photberry_get_post_option('post_additional_meta') !== '') { ?>
						<div class="photberry_post_meta_item">
							<?php echo esc_attr(photberry_get_post_option('post_additional_meta')); ?>
						</div>						
						<?php } ?>
					</div>
					...
					<?php echo photberry_get_post_formats(); ?>
					...
					if (photberry_get_prefered_option('featured_posts_status') == 'enabled') {
						photberry_featured_posts(array(
							...
							'orderby' => esc_attr(photberry_get_prefered_option('featured_posts_orderby')),
							'numberposts' => esc_attr(photberry_get_prefered_option('featured_posts_numberposts')),
							'featured_image_status' => esc_attr(photberry_get_prefered_option('featured_posts_fimage_status')),
							'excerpt' => esc_attr(photberry_get_prefered_option('featured_posts_excerpt_status')),
							'post_meta' => esc_attr(photberry_get_prefered_option('featured_posts_meta_status'))
						));
					}
					?>
				</div>
